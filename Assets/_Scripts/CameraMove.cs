...
public class CameraMove : MonoBehaviour {
	...
	void Start() {
		...
		float verExtent = Camera.main.GetComponent<Camera>().orthographicSize;
		float horExtent = verExtent * Screen.width / Screen.height;
		// float camera_width = Screen.width/2;
		minX = -map.GetComponent<Renderer>().bounds.size.x/ 2 + horExtent + map.transform.position.x;
		maxX = map.GetComponent<Renderer>().bounds.size.x/ 2 - horExtent + map.transform.position.x;
