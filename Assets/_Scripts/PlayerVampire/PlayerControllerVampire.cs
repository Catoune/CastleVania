...
//		subWeaponManager = GetComponent<SubWeaponManager> ();
	...
	void HandleOnKeyDown_B () {
		...
		gb.GetComponent<Rigidbody2D>().velocity = new Vector2 (shootSpeed, 0);
	}
...
//				float randomY = Random.Range(-1.0f, 1.0f);
				...
				GameObject gb = Instantiate (weapon, transform.position, Quaternion.identity) as GameObject;
				gb.GetComponent<Rigidbody2D>().velocity = new Vector2 (Mathf.Sin(degreeDelta), Mathf.Cos(degreeDelta)).normalized;
				gb.GetComponent<Rigidbody2D>().velocity *= 1.8f;
			}
